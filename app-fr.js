/**
 * Change Dinar - App JavaScript (Version Française)
 * Taux de change modernes et animés
 */
...
const PAGE_DESCRIPTION = 'Taux dinar algérien du marché parallèle en direct : EUR/DZD, USD/DZD, GBP/DZD et convertisseur instantané avec applications mobiles.';

// Configuration des devises (Français)
const CURRENCIES = {
    EUR: { name: 'Euro', flag: 'euro', color: 'gold' },
    USD: { name: 'Dollar US', flag: 'usa', color: 'primary' },
    GBP: { name: 'Livre Sterling', flag: 'uk', color: 'purple-400' },
    CHF: { name: 'Franc Suisse', flag: 'swi', color: 'red-500' },
    CAD: { name: 'Dollar Canadien', flag: 'canada', color: 'red-600' },
    SAR: { name: 'Riyal Saoudien', flag: 'saudi', color: 'green-500' },
    AED: { name: 'Dirham Émirati', flag: 'aed', color: 'emerald-500' },
    TRY: { name: 'Livre Turque', flag: 'turky', color: 'red-500' },
    MAD: { name: 'Dirham Marocain', flag: 'mad', color: 'red-600' },
    TND: { name: 'Dinar Tunisien', flag: 'tunes', color: 'red-500' },
    EGP: { name: 'Livre Égyptienne', flag: 'egp', color: 'yellow-500' },
    QAR: { name: 'Riyal Qatari', flag: 'qatar', color: 'purple-600' }
};
...
// Récupérer les taux de change principaux
async function fetchRates() {
    try {
        ...
        if (Array.isArray(data)) {
            ...
            const dateStr = data[0]?.price?.date || new Date().toLocaleDateString('fr-FR');
            document.getElementById('last-update').textContent = `Mis à jour: ${dateStr}`;
            updateStructuredData('fr', PAGE_DESCRIPTION);
        }
    } catch (error) {
...
// Mettre à jour les 3 cartes principales (EUR, USD, GBP)
...
function renderAllRates() {
    ...
    // Trier par priorité, exclure EUR, USD, GBP (déjà affichés)
        ...
        <div class="group relative bg-gradient-to-br from-dark-800/90 to-dark-900/90 backdrop-blur-xl rounded-2xl p-5 border border-white/5 hover:border-white/20 transition-all duration-500 hover:transform hover:-translate-y-2 hover:shadow-xl hover:shadow-primary/10"
            ...
            <div class="space-y-2">
                <div class="flex justify-between items-center p-2.5 rounded-xl bg-black/20">
                    <span class="text-slate-500 text-xs font-medium">Achat</span>
                ...
                <div class="flex justify-between items-center p-2.5 rounded-xl bg-black/10">
                    <span class="text-slate-500 text-xs font-medium">Vente</span>
...
async function fetchDigitalRates() {
    ...
    try {
        ...
        // Filtrer les éléments avec prix à 0
        ...
        const html = items.map((item, index) => {
            const name = item.currency_name || item.name || 'Inconnu';
            // Utiliser les prix EUR (eur_to_dzd_buy / eur_to_dzd_sell)
            const buy = parseFloat(item.eur_to_dzd_buy) || 0;
        ...
        container.innerHTML = html || '<p class="text-slate-500 col-span-full text-center">Aucune donnée</p>';
    } catch (error) {
        console.error('Erreur lors du chargement des taux numériques:', error);
        container.innerHTML = '<p class="text-slate-500 col-span-full text-center">Erreur de chargement</p>';
    }
}

// Récupérer les taux crypto
async function fetchCryptoRates() {
    ...
    try {
        ...
        const html = items.slice(0, 8).map((item, index) => {
            const name = item.name || item.symbol || 'Inconnu';
            const symbol = item.symbol || '';
            // Utiliser le prix en USD
            ...
            return `
                <div class="group relative bg-gradient-to-br from-dark-800/90 to-dark-900/90 backdrop-blur-xl rounded-2xl p-5 border border-white/5 hover:border-orange-500/30 transition-all duration-500 hover:transform hover:-translate-y-2 hover:shadow-xl hover:shadow-orange-500/10"
                    ...
                    <div class="flex justify-between items-end">
                        <div>
                            <span class="text-slate-500 text-xs">Prix USD</span>
        ...
        console.error('Erreur lors du chargement des taux crypto:', error);
        container.innerHTML = '<p class="text-slate-500 col-span-full text-center">Erreur de chargement</p>';
    }
}
...
function updateConverter() {
    ...
    if (rateInfoEl) {
        rateInfoEl.textContent = `1 ${currency} = ${formatNumber(rate)} DZD (Marché Parallèle)`;
    }
}

// Rafraîchir les données structurées JSON-LD avec les taux en direct
function updateStructuredData(language, description) {
    ...
    const website = {
        ...
        "@id": websiteId,
        "url": pageUrl,
        "name": "Change Dinar - Taux du Dinar Algérien",
        "inLanguage": language,
        "publisher": { "@id": orgId }
    };
    ...
    const currencyService = {
        ...
        "@id": `${pageUrl}#service`,
        "name": "Change Dinar Taux en Direct",
        "serviceType": "Taux marché parallèle",
...
// Formater les nombres USD (pour les prix crypto)
function formatNumberUsd(num) {
    if (!num || isNaN(num)) return '---';
    // Pour les grands nombres (>1000), pas de décimales
    // Pour les nombres moyens (1-1000), 2 décimales
    // Pour les petits nombres (<1), jusqu'à 6 décimales
    if (num >= 1000) {
